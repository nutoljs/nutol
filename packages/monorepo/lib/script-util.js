...
function isMonorepoPackage(pkg) {
  ...
  return Array.isArray(json.workspaces) || fs.existsSync(path.join(pkg.location, 'lerna.json'));
}
...
function runMain(currentModule, fn, ...args) {
  assert(typeof currentModule === 'object' && currentModule.filename, 'The first argument must be a module object');
