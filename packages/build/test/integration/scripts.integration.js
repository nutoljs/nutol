...
describe('build', function () {
  ...
  function cleanup() {
    ...
    run(['node', 'bin/run-clean', 'tsconfig.json', 'tsconfig.build.json', '*.tsbuildinfo', 'dist*']);
  }
  ...
  it('compiles ts files', done => {
    ...
    childProcess.on('close', code => {
      assert.equal(code, 0);
      assert(fs.existsSync(path.join(projectDir, 'dist')), 'dist should have been created');
      assert(fs.existsSync(path.join(projectDir, 'tsconfig.json')), 'tsconfig.json should have been created');
  ...
  describe('with --use-ttypescript', () => {
    it('Returns an error if ttypescript is not installed', done => {
      ...
      childProcess.on('close', code => {
        assert.equal(code, 1);
        assert.equal(processOutput, 'Error using the --use-ttypescript option - ttypescript is not installed\n');
        done();
      });
    });
  });
  ...
  it('honors tsconfig.build.json over tsconfig.json', () => {
    ...
    assert(command.indexOf('-p tsconfig.build.json') !== -1, 'project level tsconfig.build.json should be honored');
  });
  ...
  it('honors tsconfig.json if tsconfig.build.json is not present', () => {
    ...
    const run = require('../../bin/compile-package');
    const command = run(['node', 'bin/compile-package'], true);
    assert(command.indexOf('-p tsconfig.json') !== -1, 'project level tsconfig.json should be honored');
  });

  it('honors -p option for tsc', () => {
    const run = require('../../bin/compile-package');
    const command = run(['node', 'bin/compile-package', '-p', 'tsconfig.my.json'], true);
    assert(command.indexOf('-p tsconfig.my.json') !== -1, '-p should be honored');
  });

  it('honors --project option for tsc', () => {
    const run = require('../../bin/compile-package');
    const command = run(['node', 'bin/compile-package', '--project', 'tsconfig.my.json'], true);
    assert(command.indexOf('--project tsconfig.my.json') !== -1, '--project should be honored');
  });

  it('honors --target option for tsc', () => {
    const run = require('../../bin/compile-package');
    const command = run(['node', 'bin/compile-package', '--target', 'es2015'], true);
    assert(command.indexOf('--target es2015') !== -1, '--target should be honored (actual command: ' + command + ')');
  });

  it('honors --outDir option for tsc', () => {
    const run = require('../../bin/compile-package');
    const command = run(['node', 'bin/compile-package', '--outDir', './dist'], true);
    assert(command.indexOf('--outDir dist') !== -1, '--outDir should be honored');
  });

  it('removes invalid options for tsc with -b', () => {
    const run = require('../../bin/compile-package');
    const command = run(
      ['node', 'bin/compile-package', '--watch', '-b', '-v', '--xyz', '--locale', 'en_US.UTF-8'],
      true,
    );
    ...
    assert(command.indexOf('--locale en_US.UTF-8') !== -1, '--locale en_US.UTF-8 should be honored');
    assert(command.indexOf('tsc.js -b') !== -1, '-b should be the first argument');
  });

  it('runs prettier against ts files', done => {
    const run = require('../../bin/run-prettier');
    const childProcess = run(['node', 'bin/run-prettier', '-l', '**/src/*.ts'], {
  ...
  it('removes directories/files', () => {
    const run = require('../../bin/run-clean');
    const command = run(['node', 'bin/run-clean', 'tsconfig.json', 'dist'], true);
  ...
  it('does not remove directories/files outside the project', () => {
    const run = require('../../bin/run-clean');
    const command = run(
      ['node', 'bin/run-clean', '../tsconfig.json', './dist', path.join(process.cwd(), '../../api-docs')],
      true,
    );
  ...
  describe('with LERNA_ROOT_PATH', () => {
    ...
    it('sets --project option for tsc', () => {
      const run = require('../../bin/compile-package');
      const command = run(['node', 'bin/compile-package'], true);
      const tsConfig = path.relative(repoRoot, path.join(__dirname, './fixtures/tsconfig.json'));
      assert(command.indexOf(`-p ${tsConfig}`) !== -1, '-p should be set relative to the monorepo root');
    });

    afterAll(() => delete process.env.LERNA_ROOT_PATH);
  });

  describe('resolveCLIFromProject()', () => {
    it('returns undefined if the CLI is not found in project deps', () => {
      assert.equal(utils.resolveCLIFromProject('mocha/bin/mocha', projectDir), undefined);
    });
