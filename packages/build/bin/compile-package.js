...
function run(argv, options) {
  ...
  const runnerIsLbttsc = runnerName.includes('nb-ttsc');
  ...
  const isCopyResourcesSet = utils.isOptionSet(compilerOpts, '--copy-resources');
  ...
  if (useTtsc) {
    try {
      ...
      if (isUseTtscSet) {
        console.error('Error using the --use-ttypescript option - ttypescript is not installed');
      } else {
        console.error('Error using nb-ttsc - ttypescript is not installed');
      }
      process.exit(1);
    }
  }
...
function validArgsForBuild(args) {
  ...
  buildOptions.forEach(opt => {
     ...
     * type: "boolean",
     */
    const options = opt.type === 'boolean' ? validBooleanOptions : validValueOptions;
...
function copyResources(rootDir, packageDir, tsConfigFile, outDir, options) {
  ...
  if (!tsConfigFile) {
    console.warn('Ignoring --copy-resources option - no tsconfig file was found.');
    return;
  }
  ...
  if (!outDir) {
    ...
    if (!outDir) {
      console.warn('Ignoring --copy-resources option - outDir was not configured.');
      return;
    }
  }

  const dirs = tsConfig.include ? tsConfig.include.join('|') : ['src', 'test'].join('|');

  const compilerRootDir = (tsConfig.compilerOptions && tsConfig.compilerOptions.rootDir) || '';
