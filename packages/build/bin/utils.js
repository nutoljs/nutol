 ...
 *   dependencies of the current project first instead of `@nutol/build`
 */
...
function resolveCLIFromProject(cli, projectRootDir = getPackageDir()) {
  ...
  if ((pkg.devDependencies && pkg.devDependencies[cliPkg]) || (pkg.dependencies && pkg.dependencies[cliPkg])) {
...
function runShell(command, args, options) {
  ...
  child.on('close', (code, signal) => {
    ...
    if (code > 0 || signal) {
      console.warn('Command aborts (code %d signal %s): %s %s.', code, signal, command, args.join(' '));
    }
...
function mochaConfiguredForProject() {
  const configFiles = ['.mocharc.js', '.mocharc.json', '.mocharc.yaml', '.mocharc.yml'];
